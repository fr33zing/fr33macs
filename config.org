...
Code actions are provided by lsp-mode and lsp-ltex-plus via different
commands, this bridges the gap between them.
...
(setq my:point-action-sources
   '((:name "English (ltex-ls-plus)"
     ...
     (:name "Code actions (lsp-mode)"
      ...
      :check (lambda () (if (boundp 'lsp-ui-sideline--code-actions)
                            lsp-ui-sideline--code-actions)))))

(defun my:choose-point-action ()
  (interactive)
  (let (( sources (seq-remove (lambda (src) (null (funcall (plist-get src :check))))
                              my:point-action-sources)))
    (if sources
      (if (= 1 (length sources))
...
(use-package flycheck
  ...
  (defun set-flycheck-color (type fg-name bg)
  ...
  ;                                     5% opacity
  (set-flycheck-color "error"   'red    "#453243")
  (set-flycheck-color "warning" 'yellow "#454144")
  (set-flycheck-color "info"    'mauve  "#3c3653"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(use-package dimmer
  :vc "https://github.com/chrhansk/dimmer.el.git"
  :ensure t
  :init
...
(use-package lsp-ltex-plus
  ...
  :custom (lsp-ltex-plus-diagnostics-provider 'flycheck))
#+END_SRC
...
(use-package srefactor
  :ensure t
  :vc (:url "https://github.com/tuhdo/semantic-refactor" :rev :newest))
#+END_SRC
